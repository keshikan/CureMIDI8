...
// for Cure Series
#define MIDI_BUFFER_SIZ (4096)//FIFO buffer byte size for midi message buffer. need 2^n
#define MIDI_BUFFER_FULL_THR (4000)

bool is_usb_midi_buffer_full = false;
...
UM_MidiAnalysisStatus analyzed_status[MIDI_IN_JACK_NUM];
UM_MIDIEvent midi_event[MIDI_IN_JACK_NUM];	//received midi data
...
FUNC_STATUS midiInit()
{
	...
	for(i=0; i<MIDI_IN_JACK_NUM; i++){
		...
		for(j=0; j<UM_MIDI_SENDDATA_MAX; j++){
...
inline uint16_t getUsbRxBufUsedSize(uint8_t cable_num)
{
	return _cureRingBufferU8GetUsedSize(&rbuf_usb_rx[cable_num]);
}
...
bool isAllUsbRxBufEmpty()
{
	for(uint32_t i=0; i<MIDI_OUT_JACK_NUM; i++){
...
bool isUsbRxBufferFull()
{
	return is_usb_midi_buffer_full;
}

void usbMidiBufferFlush(uint8_t cable_num)
{
	cureRingBufferU8Flush(&rbuf_usb_rx[cable_num]);
	is_usb_midi_buffer_full = false;
}
...
static uint16_t MIDI_DataRx(uint8_t *msg, uint16_t length){
  ...
  //buffer full judge
  if(MIDI_BUFFER_FULL_THR < getUsbRxBufUsedSize(0)){
	  is_usb_midi_buffer_full = true;
  }
...
bool midiIsEventGenerated(uint8_t cable_num)
{
	...
	if( upper_half_byte & 0x80 ){//0x80-0xFF:status byte
		...
		switch(upper_half_byte){
			...
			case 0xF0://0xF0-0xFF:system message
				switch(rx_midi_msg[cable_num]){
					...
					case 0xF0://SysEx Start
						...
						analyzed_status[cable_num].type = UM_MSG_SYSEX;
						analyzed_status[cable_num].stat = UM_WAIT_SYSTEM_DATA;
						break;

					case 0xF7://SysEx End
						midi_event[cable_num].midi_byte[ analyzed_status[cable_num].data_idx++ ] = rx_midi_msg[cable_num];
						midi_event[cable_num].length = analyzed_status[cable_num].data_idx;
						analyzed_status[cable_num].stat = UM_END_ANALYSIS;
						break;

					case 0xF2://Song Position
						midi_event[cable_num].midi_byte[0] = rx_midi_msg[cable_num];
						analyzed_status[cable_num].type = UM_MSG_THREE_BYTE;
						analyzed_status[cable_num].stat = UM_WAIT_DATA1;
						break;

					case 0xF1://Time Code
					case 0xF3://Song Select
						midi_event[cable_num].midi_byte[0] = rx_midi_msg[cable_num];
						analyzed_status[cable_num].type = UM_MSG_TWO_BYTE;
						analyzed_status[cable_num].stat = UM_WAIT_DATA1;
						break;
					...
					case 0xFF://Reset
						...
						analyzed_status[cable_num].type = UM_MSG_ONE_BYTE;
						analyzed_status[cable_num].stat = UM_END_ANALYSIS;
						break;
				}
			...
			default:
				analyzed_status[cable_num].type = UM_MSG_NOTHING;
				analyzed_status[cable_num].stat = UM_START_ANALYSIS;
				analyzed_status[cable_num].is_system_common = false;
				break;
		}
		...
		switch(analyzed_status[cable_num].stat){
			...
			case UM_WAIT_DATA1:
				...
				if(UM_MSG_THREE_BYTE == analyzed_status[cable_num].type ){
					analyzed_status[cable_num].stat = UM_WAIT_DATA2;
				}else if( UM_MSG_TWO_BYTE == analyzed_status[cable_num].type ){
			...
			case UM_WAIT_SYSTEM_DATA:
				midi_event[cable_num].midi_byte[ analyzed_status[cable_num].data_idx++ ] = rx_midi_msg[cable_num];

				if(analyzed_status[cable_num].data_idx > (UM_MIDI_SENDDATA_MAX - 1) ){
					analyzed_status[cable_num].stat = UM_END_ANALYSIS;
				}
				break;

			case UM_END_ANALYSIS://running status:When status byte is omitted.
				midi_event[cable_num].midi_byte[1] = rx_midi_msg[cable_num];
				if(UM_MSG_THREE_BYTE == analyzed_status[cable_num].type){
					analyzed_status[cable_num].stat = UM_WAIT_DATA2;
				}else if(UM_MSG_TWO_BYTE == analyzed_status[cable_num].type){
	...
	if(UM_END_ANALYSIS == analyzed_status[cable_num].stat){
		return true;
	}else{
		return false;
	}
...
void midiGenerateUsbPacket(uint8_t cable_num)
{
	...
	switch(analyzed_status[cable_num].type){
		...
		case UM_MSG_TWO_BYTE:
		case UM_MSG_THREE_BYTE:
			//byte 0: cable number + code index number
...
void midiInProcess(){
	...
	for(uint32_t cable_num=0; cable_num<MIDI_IN_JACK_NUM; cable_num++){
		while( FUNC_SUCCESS == midiGetFromJackRx(cable_num) ){
			if( midiIsEventGenerated(cable_num) ){// Generate MIDI event from UART buffer.
